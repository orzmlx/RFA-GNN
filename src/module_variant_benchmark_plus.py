...
def main():
    ...
    from module_graph_recipe_plus import (
        build_gene_module_matrix,
        build_nmf_expression_modules,
        build_ppi_community_modules,
        build_signed_module_adj,
        build_tf_regulon_modules,
        coarsen_edges_to_modules,
        dedup_modules,
        load_landmark_entrez,
        load_omnipath_ppi_edges,
        load_omnipath_tf_edges,
        load_symbol_to_entrez,
        select_modules_cover_greedy,
    )
    ...
    node_list = load_landmark_entrez(landmark_path)
    ...
    data_full = load_rfa_data(
        ...
        cell_lines=(None if str(args.cell_line).upper() == "ALL" else args.cell_line),
        ctl_residual_pool_size=0,
    )

    X_ctl = np.asarray(data_full["X_ctl"], dtype=np.float32)
    y = np.asarray(data_full["y_delta"], dtype=np.float32)
    X_drug = np.asarray(data_full["X_drug"], dtype=np.float32)
    fp = np.asarray(data_full["X_fingerprint"], dtype=np.float32)
    drug_ids = np.asarray(data_full["drug_ids"], dtype=str)
    cell_names = np.asarray(data_full["cell_names"], dtype=str)
    loss_mask = np.asarray(data_full["loss_mask"], dtype=np.float32)
    ...
    y_nmf = y
    if int(args.max_samples_for_nmf) > 0 and len(y_nmf) > int(args.max_samples_for_nmf):
        rng = np.random.default_rng(int(args.seed))
        idx = rng.choice(len(y_nmf), size=int(args.max_samples_for_nmf), replace=False)
        y_nmf = y_nmf[idx]
    ...
    all_mods = dedup_modules(tf_mods + ppi_mods + nmf_mods, max_overlap_jaccard=0.95)
    ...
    A_pos, A_neg, A_undir = coarsen_edges_to_modules(gene2idx, modules, edges_directed_signed, edges_undirected, keep_top_per_src=50)
    A_kk = build_signed_module_adj(A_pos, A_neg, A_undir)
    print(f"Modules: {int(M_gk.shape[1])} | Module edges nnz: {int(np.count_nonzero(A_kk))}")
    ...
    for name, model in schemes:
        ...
        for ep in range(int(args.epochs)):
            ...
            pcc = samplewise_pcc(y_test_full, pred_eval, loss_mask)
            mse = samplewise_mse(y_test_full, pred_eval, loss_mask)
            best = max(best, pcc)
