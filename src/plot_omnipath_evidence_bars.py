...
def main():
    ...
    counts = {}
    for m in metrics:
        counts[("interactions", m)] = _bin_counts(df_i[m])
        counts[("tf_regulons", m)] = _bin_counts(df_t[m])

    bins = counts[("interactions", metrics[0])].index.tolist()
    ...
    for ax, m in zip(axes, metrics):
        ...
        w = 0.38
        c_i = counts[("interactions", m)].values
        c_t = counts[("tf_regulons", m)].values
        ax.bar(
            [xi - w / 2 for xi in x],
            c_i,
            width=w,
            color=to_rgba(colors[0], 0.75),
            edgecolor=edge,
            linewidth=0.6,
            label="interactions",
        )
