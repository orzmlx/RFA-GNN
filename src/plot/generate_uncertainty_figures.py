...
LANDMARK_PATH = PATHS["data_dir"] / "landmark_genes.json"
SPLIT_PATHS = {
...
def save_summary_figure(runs):
    ...
    fig.tight_layout()
    png_path = FIG_DIR / "uncertainty_summary_placeholder.png"
    pdf_path = FIG_DIR / "uncertainty_summary_placeholder.pdf"
...
def save_case_study_figure(runs):
    ...
    png_path = FIG_DIR / "uncertainty_case_study_placeholder.png"
    pdf_path = FIG_DIR / "uncertainty_case_study_placeholder.pdf"
    fig.savefig(png_path, bbox_inches="tight")
    fig.savefig(pdf_path, bbox_inches="tight")
    plt.close(fig)
    return png_path, pdf_path
...
def generate_all():
    runs = load_runs()
    outputs = []
    outputs.extend(save_summary_figure(runs))
    outputs.extend(save_case_study_figure(runs))
    return outputs
