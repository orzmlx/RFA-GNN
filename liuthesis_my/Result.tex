% !TeX root = thesis.tex
\chapter{Results}
This chapter presents the main results for the proposed model and the baseline models under the warm, cold drug target, and cold cell splits.

\section{Evaluation framework}
\noindent All experiments in this chapter use the L1000 landmark setting with 978 genes. For every sample, the prediction target is the response vector $y = x_{\mathrm{trt}} - x_{\mathrm{ctl}}$. All learned models use the matched control profile and the drug input. The proposed model also uses cell context information.
...
\noindent This ablation study is separate from the main benchmark comparison. It includes two focused analyses. One compares two ways of providing cell information to the proposed model. The other examines the contribution of the drug target input by testing what happens when that input is removed at test time.
...
\caption[Warm Data Efficiency]{Warm split data efficiency results. The left panel shows test PCC and the right panel shows test MSE for DeepCOP, GSNN, and UPert across the displayed training budgets.}
\label{fig:data_efficiency_placeholder}
\end{figure}

\noindent Figure~\ref{fig:data_efficiency_placeholder} shows that UPert remains strongest on PCC across the displayed warm split budgets, and its advantage is already visible at smaller budgets. DeepCOP also improves steadily as more data are added, with higher PCC and lower MSE across the shown budgets. GSNN changes less over the same range.
\section{Interpretation and case studies}
\noindent Average metrics show overall performance, but they do not show how the model uses the graph in one specific example. The following cases are selected from UPert. Here attention means the weight that the model gives to one graph edge during message passing. A larger weight means that the model uses that edge more strongly in the final prediction. These weights are not treated as proof of a real biological mechanism. The analysis asks whether highly weighted edges are easier to understand when the prediction is accurate, and whether the model keeps giving high weights to similar edges across different settings. Table~\ref{tab:case_study_terms} gives short explanations of the main biology terms used in this section. These short notes are only meant to help reading. They do not replace the full biological background of each gene or drug class.
