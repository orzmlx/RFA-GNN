...
\noindent The evaluation compares DeepCOP as the dense baseline, GSNN \cite{evans2024gsnn} as the graph based baseline, and two versions of Uncertainty Perturbation (UPert). The no-CF version is the standard UPert model without the counterfactual constraint. The CF version keeps the same architecture but adds the counterfactual drug constraint described in the methodology chapter. Some tables and figures also include simple mean baselines, namely mean\_global, mean\_cell, and mean\_drug, where the latter two use global fallback when the corresponding cell line or drug is unavailable in the training split.
...
\noindent Error metrics measure how close the predicted values are to the observed values. The main error metric here is mean squared error (MSE), where a lower score means a better prediction. Table~\ref{tab:mse_results} reports the benchmark comparison with both UPert variants and the simple mean baselines, while Figure~\ref{fig:benchmark_mse_results} keeps the mean baseline plot view. In the latest exports, no-CF UPert gives the lowest MSE among the learned models in warm and cold drug target, while CF UPert becomes the stronger learned UPert variant in cold cell. However, the mean\_drug baseline is still lowest overall in cold cell.
...
\noindent Table~\ref{tab:pcc_results} reports the benchmark comparison with both UPert variants and the simple mean baselines, while Figure~\ref{fig:benchmark_pcc_results} keeps the mean baseline plot view. In the latest exports, no-CF UPert remains stronger on PCC in warm and cold drug target, whereas CF UPert becomes stronger in cold cell. The strongest learned baseline changes by split. DeepCOP is slightly stronger than GSNN in warm, while GSNN is clearly stronger in cold drug target and cold cell. Another clear result is that PCC decreases for all learned models from warm to cold cell.
...
\noindent Across the three splits, the lowest PCC values still appear in cold cell for all models. The ranking by PCC and the ranking by sign accuracy now follow the same split dependent pattern. No-CF UPert stays stronger in warm and cold drug target, while CF UPert is stronger in cold cell. The PCC margin is largest in warm and cold drug target and smaller in cold cell. The gene level plots below keep the main no-CF benchmark view, so they should be read together with the split tables above rather than as a separate comparison of the two UPert variants.

\subsection{Gene distribution analysis}
\noindent Figure~\ref{fig:xpert_style_gene_distribution} gives a view of model output across selected genes for the main no-CF UPert run. The figure has three rows, one for each split, and each row shows the distribution of $x_{\mathrm{deg}}$ for twelve genes with strong true response.

\noindent The pattern is consistent with the no-CF summary metrics reported above. In warm and cold drug target, the no-CF UPert distributions are usually closest to the ground truth distributions across the selected genes. In cold cell, the agreement becomes weaker for all models, but no-CF UPert still tracks the ground truth more closely than the two baselines in most shown genes. The figure therefore remains useful as a qualitative view of the main benchmark run, even though the CF variant is stronger on several cold cell metrics in the updated tables.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth]{figures/xpert_style_gene_distribution.png}
\caption[Gene Distribution by Split]{Distribution across selected genes in the three splits. The figure has three rows, one for each split. Each row shows $x_{\mathrm{deg}}$ for twelve genes with strong true response. Ground truth, DeepCOP, GSNN, and UPert are shown for each gene.}
...
\subsubsection{Drug input check}
\noindent This check asks whether the model still relies on the drug input at test time. In this subsection, CF means the counterfactual constraint introduced in the method chapter. During training, that constraint compares the normal drug input with a counterfactual version in which the drug input is removed, in order to discourage the model from ignoring the drug signal. The comparison is shown in two steps. Table~\ref{tab:drug_zero_sanity_no_cf} compares the normal drug setting and the zero drug setting for the no-CF variant. Table~\ref{tab:drug_zero_sanity_cf} shows the same comparison for the CF variant. Here $\Delta$ is defined as zero drug minus test, so a positive $\Delta$MSE and a negative $\Delta$PCC both indicate performance degradation. The main pattern is that the CF variant is clearly more sensitive to removing the drug input in all three splits, and the largest gap appears in cold cell. In the standard test setting, CF also improves both MSE and PCC in cold cell.
...
\subsection{Prediction band diagnostics}
\noindent Under a Gaussian interpretation, the band $\mu \pm 2\sigma$ can be read as an approximate 95\% prediction interval. To check this more directly, the empirical coverage is calculated as the fraction of target genes whose true values fall inside $\mu \pm 2\sigma$. Table~\ref{tab:uncertainty_empirical_coverage} reports the full test set together with the best top10, top20, and top100 samples ranked by sample level NLL from low to high. This makes it possible to compare overall calibration with the best calibrated subsets. In the uncertainty enabled no-CF UPert run in \texttt{outputs/no\_cf\_0523}, warm and cold cell remain close to the nominal 95\% level on the full test set, while cold drug target is clearly lower. The best NLL subsets are still fairly similar across all three splits. This suggests that cold drug target has weaker overall calibration than the other two splits, but no longer shows the extreme NLL failure seen in the earlier CF-centered run.

\begin{table}[htbp]
\centering
\caption[Uncertainty Diagnostics by Best NLL Subset]{Uncertainty diagnostics on the full test set and on the best top10, top20, and top100 samples ranked by sample level NLL from low to high. Coverage is computed for the prediction band $\mu \pm 2\sigma$.}
\label{tab:uncertainty_empirical_coverage}
\begin{tabular}{llcccc}
\hline
Split & Subset & NLL & MSE & PCC & Coverage \\
\hline
Warm & Full test & 1.1927 & 0.8260 & 0.5583 & 95.64\% \\
Warm & Best top10 & 0.6392 & 0.2419 & 0.5472 & 98.81\% \\
Warm & Best top20 & 0.6600 & 0.2281 & 0.5446 & 99.10\% \\
Warm & Best top100 & 0.7134 & 0.2333 & 0.5506 & 99.29\% \\
\hline
Cold drug target & Full test & 1.5931 & 1.2643 & 0.5220 & 88.62\% \\
Cold drug target & Best top10 & 0.6323 & 0.1987 & 0.5954 & 99.15\% \\
Cold drug target & Best top20 & 0.6537 & 0.2156 & 0.5677 & 99.07\% \\
Cold drug target & Best top100 & 0.7254 & 0.2754 & 0.5665 & 98.80\% \\
\hline
Cold cell & Full test & 1.2101 & 0.8256 & 0.4795 & 95.91\% \\
Cold cell & Best top10 & 0.7395 & 0.2097 & 0.5832 & 99.63\% \\
Cold cell & Best top20 & 0.7485 & 0.2143 & 0.5824 & 99.69\% \\
Cold cell & Best top100 & 0.7769 & 0.2459 & 0.5630 & 99.57\% \\
\hline
\end{tabular}
\end{table}
...
\noindent Average metrics show overall performance, but they do not show how the model uses the graph in one specific example. The following cases are selected from UPert. Here attention means the weight that the model gives to one graph edge during message passing. A larger weight means that the model uses that edge more strongly in the final prediction. These weights are not treated as proof of a real biological mechanism. The analysis instead asks two simple questions. Are the highly weighted edges easier to understand when the prediction is accurate, and does the model keep giving high weights to similar edges across different settings.
