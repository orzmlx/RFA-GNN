...
This thesis uses two data sources. One is a large perturbation gene expression dataset. Another is the cellular interaction database, which provides known information about how genes and proteins are linked. Unless otherwise stated, figures from this chapter were created by the author.
...
\subsubsection{Prediction target construction}
\label{sec:prediction_target_construction}
\noindent After a treatment profile and a matched control profile are paired, the response target is constructed as
\begin{equation}
y = x_{\mathrm{trt}} - x_{\mathrm{ctl}}.
\end{equation}
\noindent Here $x_{\mathrm{trt}}$ is the treated expression vector and $x_{\mathrm{ctl}}$ is the matched control expression vector. The target therefore represents the change after drug treatment rather than the full expression state of the sample.

\noindent This construction is used because the matched control profile already contains the baseline state of the cell. By subtracting it from the treated profile, the target focuses more directly on the drug related response. This also makes the prediction task more consistent with the paired design used in preprocessing, where each treatment example is linked to a control measured in a similar experimental setting.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.76\linewidth]{figures/overview_of_omnipath.png}
